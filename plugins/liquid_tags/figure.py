...
Syntax
------
{% figure [class name(s)] [http[s]:/]/path/to/image [width [height]] [title text | "title text" ["alt text"]] %}

Examples
--------
{% figure /images/ninja.png Ninja Attack! %}
{% figure left half http://site.com/images/ninja.png Ninja Attack! %}
{% figure left half http://site.com/images/ninja.png 150 150 "Ninja Attack!" "Ninja in attack posture" %}

Output
------
<figure><img src="/images/ninja.png"></figure>
<figure><img class="left half" src="http://site.com/images/ninja.png" title="Ninja Attack!" alt="Ninja Attack!"></figure>
<figure><img class="left half" src="http://site.com/images/ninja.png" width="150" height="150" title="Ninja Attack!" alt="Ninja in attack posture"></figure>
...
SYNTAX = '{% figure [class name(s)] [http[s]:/]/path/to/image [width [height]] [title text | "title text" ["alt text"]] %}'
...
def img(preprocessor, tag, markup):
    ...
    return "<figure><img {0}>{1}</figure>".format(' '.join('{0}="{1}"'.format(key, val)
                                       for (key, val) in six.iteritems(attrs)), '<figcaption>%s</figcaption>' % attrs['title'] if 'title' in attrs else '')
